...
On a [soft navigation](/docs/app/routing/linking-and-navigating#soft-navigation) - Next.js will render the slot's previously active state, even if it doesn't match the current URL.

#### [Hard Navigation](#hard-navigation)

On a [hard navigation](/docs/app/routing/linking-and-navigating#hard-navigation) - a navigation that requires a full page reload - Next.js will first try to render the unmatched slot's `default.js` file. If that's not available, a 404 gets rendered.
...
> More information on modals is covered in the [Intercepting Routes](/docs/app/routing/intercepting-routes) section.
