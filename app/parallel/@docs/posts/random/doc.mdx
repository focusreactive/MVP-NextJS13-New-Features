...
See [`revalidate`](https://nextjs.org/docs/app/api-reference/file-conventions/route-segment-config#revalidate) for other possible configuration options.
...
- `fetch()` has already been patched to include support for `cache()` automatically, so you don't need to wrap functions that use `fetch()` with `cache()`. See [automatic request deduping](https://nextjs.org/docs/app/building-your-application/data-fetching#automatic-fetch-request-deduping) for more information.
...
- Using `cache` allows shared data fetching without having to know if a parent layout was [rendered during this request](https://nextjs.org/docs/app/building-your-application/routing#partial-rendering).
