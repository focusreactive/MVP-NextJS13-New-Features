...
- On the server, the cache lasts the lifetime of a server request until the rendering process completes.
  ...
  - This optimization also applies during [static generation](https://nextjs.orghttps://nextjs.org/docs/app/building-your-application/rendering#static-rendering).
...
`fetch()` is a Web API used to fetch remote resources that **returns a promise**. React extends `fetch` to provide [automatic request deduping](https://nextjs.orghttps://nextjs.org/docs/app/building-your-application/data-fetching#automatic-fetch-request-deduping), and Next.js extends the `fetch` options object to allow each request to set its own [caching and revalidating](https://nextjs.orghttps://nextjs.org/docs/app/building-your-application/data-fetching/caching).
...
- [`cookies()`](https://nextjs.orghttps://nextjs.org/docs/app/api-reference/functions/cookies)
- [`headers()`](https://nextjs.orghttps://nextjs.org/docs/app/api-reference/functions/headers)
...
By default, `fetch` will automatically fetch and [cache data](https://nextjs.orghttps://nextjs.org/docs/app/building-your-application/data-fetching/caching) indefinitely.
...
To revalidate [cached data](https://nextjs.orghttps://nextjs.org/docs/app/building-your-application/data-fetching/caching) at a timed interval, you can use the `next.revalidate` option in `fetch()` to set the `cache` lifetime of a resource (in seconds).

```ts
fetch('https://...', { next: { revalidate: 10 } });
```

See [Revalidating Data](https://nextjs.orghttps://nextjs.org/docs/app/building-your-application/data-fetching/revalidating) for more information.
...
To improve the user experience, you can add a [suspense boundary](https://nextjs.orghttps://nextjs.org/docs/app/building-your-application/routing/loading-ui-and-streaming) to break up the rendering work and show part of the result as soon as possible:
...
Take a look at the [preloading pattern](https://nextjs.orghttps://nextjs.org/docs/app/building-your-application/data-fetching/caching#preload-pattern-with-cache) for more information on improving components structure.
...
By fetching data in a [layout](https://nextjs.orghttps://nextjs.org/docs/app/building-your-application/routing/pages-and-layouts), rendering for all route segments beneath it can only start once the data has finished loading.
...
In cases where you cannot use `fetch` but still want to control the caching or revalidating behavior of a layout or page, you can rely on the [default caching behavior](https://nextjs.orghttps://nextjs.org/docs/app/building-your-application/data-fetching/fetching#default-caching-behavior) of the segment or use the [segment cache configuration](#segment-cache-configuration).
...
> **Good to know:** Dynamic functions like [`cookies()`](https://nextjs.orghttps://nextjs.org/docs/app/api-reference/functions/cookies) and [`headers()`](https://nextjs.orghttps://nextjs.org/docs/app/api-reference/functions/headers) will make the route segment dynamic.

### [Segment Cache Configuration](#segment-cache-configuration)

As a temporary solution, until the caching behavior of third-party queries can be configured, you can use [segment configuration](https://nextjs.orghttps://nextjs.org/docs/app/api-reference/file-conventions/route-segment-config#configrevalidate) to customize the cache behavior of the entire segment.
